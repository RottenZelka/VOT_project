...
const SchoolSignUpScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        const response = await axios.get('https://localhost:3000/countries');
    ...
    const fetchSchoolTypes = async () => {
      try {
        const response = await axios.get('https://localhost:3000/school_types');
  ...
  const handleSignUp = async () => {
    try {
      const response = await axios.post('http://localhost:3000/signupSchool', {
        abbreviation,
        password,
